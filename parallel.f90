...
!***********************************************************************
! Here is the matrix-vector SUBROUTINE, Here we paralelizate the do
! loops
!***********************************************************************
  ...
  REAL(KIND=8), DIMENSION(dm,dm) :: mat_a
      ...
      !$acc data copyin(mat_a(dm,dm),x0(dm)) copy(x1(dm))
      !$acc kernels
      DO i = 1, dm
          tmps     = 0.d0
          DO j = 1, dm
              tmps = tmps + mat_a(i,j)*x0(j)
          ENDDO
          x1(i) = tmps
      ENDDO
      !$acc end kernels
...
ENDPROGRAM power_method_parallel
